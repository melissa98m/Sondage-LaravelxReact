...
function DeleteQuestion(props) {
    ...
    const [delQuestion, setShowDelete] = useState(false);
    const [toast, setShowToast] = useState(false);
    const [toastMessage, setToastMessage] = useState({});

    let deleteQuestion = async (e) => {
        e.preventDefault();
        try {
            let res = await axios.delete('http://127.0.0.1:8000/api/questions/' + oneQuestion.id)
            if (res.status === 200) {
                const foundIndex = props.deleteValue.data.findIndex(x => x.id === oneQuestion.id);
    ...
    return(<Box>
            <Button
                ...
                onClick={ () => {
                    setShowDelete(true)
                    setOneQuestion({id: props.deleteValue.id, value: props.deleteValue.value} )
                } }
            >
            ...
            >
                <Box className="modal-type" sx={{bgcolor: 'background.default'}}>
                    ...
                    <FormControl>
                        <Box>
                            êtes vous sur de vouloir supprimer le Question : {oneQuestion.value}?
                        </Box>
                        <Box className="action-button">
                            <Button sx={{m: 3}} type="submit" variant="contained" onClick={deleteQuestion}>Envoyer</Button>
